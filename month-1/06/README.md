...
**XXE** (XML external entity, или инъекция внешних сущностей XML) - уязвимость, позволяющая злоумышленнику вмешиваться в обработку XML-документов в приложении. Такая атака может привести к:

* вызовам отказа в обслуживании

* доступу к локальным файлам

* выполнению подделки запросов на стороне сервера (SSRF)

* ряду других проблем, типа сканирования портов мнашины, на которой запущен XML-анализатор или инъекции команд
...
Средства защиты от такого рода атак включают ограничение памяти, выделенной в отдельном анализаторе, если допустима потеря документа, или символическую обработку объектов и их расширение только тогда, когда будет использоваться их содержимое, а не сразу всего документа.
...
Проверим работу XXE с использованием механизма XInclude на примере лабораторной работы https://portswigger.net/web-security/xxe/lab-xinclude-attack
...
Мы видим, что отправляется не готовый XML-документ, а форма с двумя параметрами, которые будут встроены в итоговый документ уже на стороне сервера. Значит мы не можем контролировать XML-документ полностью, и можем попытаться встроить свой код, используя XInclude:
...
Подставив код, указанный выше как значение параметра productId получим ответ:
...
Основной причиной возможности таких атак является XML-анализатор, который сконфигурирован недостаточно безопасным образом.

Основными способами защиты от XXE-атак являются:
